...
export default function SearchPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedFilters.category.length > 0) {
      results = results.filter((product) => selectedFilters.category.includes(product.category))
    }

    // Filter by brand
    if (selectedFilters.brand.length > 0) {
      results = results.filter((product) => selectedFilters.brand.includes(product.brand))
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        {/* Search Header */}
        <div className="mb-8">
          <h1 className="text-2xl lg:text-3xl font-bold mb-4">Search Products</h1>
